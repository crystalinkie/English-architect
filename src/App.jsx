    ...
    <div style={{ background: "#FFF9F2", minHeight: "100vh" }}>
      ...
      <div style={{ padding: "28px 20px" }}>
        <div style={{ background: "#1A1A2E", borderRadius: 20, padding: "26px 22px", position: "relative", overflow: "hidden" }}>
          ...
          <div onClick={() => setShowPopup(true)} style={{ marginTop: 22, background: t.color, borderRadius: 12, padding: "15px", textAlign: "center", cursor: "pointer", boxShadow: `0 4px 18px ${t.glow}`, fontSize: 15, fontWeight: 700, color: "white" }}>
            📋 구글 폼으로 신청하기
          </div>
          {showPopup && (
            <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: "0 24px" }}>
              <div style={{ background: "white", borderRadius: 20, padding: "28px 24px", width: "100%", maxWidth: 340, textAlign: "center" }}>
                <div style={{ fontSize: 22, marginBottom: 10 }}>📋</div>
                <div style={{ fontSize: 17, fontWeight: 900, color: "#1A1A2E", marginBottom: 8 }}>프리미엄 리포트 신청</div>
                <div style={{ fontSize: 13, color: "#888", lineHeight: 1.7, marginBottom: 20 }}>아래 링크를 복사해서<br />브라우저에 붙여넣기 해주세요!</div>
                <div style={{ background: "#F5F5F5", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#333", fontWeight: 600, wordBreak: "break-all", marginBottom: 20, userSelect: "all" }}>
                  forms.gle/W7YeRMADr98HXCVt9
                </div>
                <div onClick={() => {
                  navigator.clipboard.writeText("https://forms.gle/W7YeRMADr98HXCVt9").catch(() => {});
                  setShowPopup(false);
                }} style={{ background: t.color, borderRadius: 12, padding: "13px", fontSize: 14, fontWeight: 700, color: "white", cursor: "pointer", marginBottom: 10 }}>
                  링크 복사하기
                </div>
                <div onClick={() => setShowPopup(false)} style={{ fontSize: 13, color: "#aaa", cursor: "pointer", padding: "6px" }}>닫기</div>
              </div>
            </div>
          )}
      ...
      <div style={{ padding: "0 20px 48px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
          <div style={{ background: "#F0EAE0", borderRadius: 40, padding: "10px 22px", fontSize: 13, fontWeight: 700, color: "#1A1A2E", cursor: "pointer" }}>🔗 결과 공유하기</div>
